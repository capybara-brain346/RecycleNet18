...
def chat_response(object: str) -> None:
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            (
                "human",
                """
                Tell me how can I recycle or reuse {object}. 
                Return your response in the following format: {output_schema} but don't format it as json, instead format it as natural language.
                """,
            ),
        ]
    )
    ...
    response = run.stream({"object": object, "output_schema": output_schema})
    st.write(response)
